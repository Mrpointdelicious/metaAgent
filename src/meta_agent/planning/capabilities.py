...
class HistoryArgs(StrictModel):
    ...
    training_state: Literal["all", "completed", "not_started", "not_completed",
                            "execution_unknown", "voided"] = "all"
...
def capability_specs(settings: Settings) -> dict[str, CapabilitySpec]:
    specs = [
        ...
        CapabilitySpec("rehab.resolve_session", ResolveArgs, requires_patient=True,
                       provides=("session_ref", "evidence_id")),
        CapabilitySpec("rehab.session", SessionArgs, requires_patient=True,
                       provides=("session_ref", "evidence_id")),
        CapabilitySpec("rehab.single_report", ReportArgs, "prepare_artifact", True,
                       provides=("artifact_ref",)),
        CapabilitySpec("rehab.trend", TrendArgs, requires_patient=True, provides=("window",)),
        CapabilitySpec("rehab.trend_report", TrendArgs, "prepare_artifact", True,
                       provides=("artifact_ref",)),
        CapabilitySpec("scene.resolve", SceneArgs, requires_space=True,
                       provides=("action_ref",), enabled=settings.scene_enabled),
        CapabilitySpec("scene.dispatch", DispatchArgs, "emit_frontend_action",
                       requires_space=True, enabled=settings.scene_actions_enabled),
        CapabilitySpec("doctors.search", EmptyArgs, requires_space=True,
                       enabled=settings.doctors_enabled),
        CapabilitySpec("knowledge.search", KnowledgeArgs,
                       enabled=bool(settings.knowledge_corpus_path)),
