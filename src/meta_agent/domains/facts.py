...
class FactBuilder:
    def __init__(
        self, evidence: EvidenceEnvelope, record_ref: str | None = None, observed_at: Any = None
    ) -> None:
    ...
    def add(
        ...
        *,
        ...
        if status not in {"valid", "missing", "invalid", "unknown", "not_comparable"}:
            status = "unknown"
        fact = Fact(
            fact_id="fact_" + fingerprint([self.evidence.evidence_id, path])[:24],
            ...
            source_version=self.evidence.source_version,
    ...
    def quality(self, prefix: str = "/data/quality") -> None:
        ...
        if not exists or not isinstance(quality, dict) or not quality:
            self.add(prefix + "/overall_status", "quality", "数据质量", required=True)
            return
        for field, label in [
            ("overall_status", "数据质量"),
            ("trend_eligibility", "趋势比较资格"),
            ("patient_message", "质量说明"),
        ...
        for index, issue in enumerate(quality.get("issues") or []):
            if isinstance(issue, dict) and issue.get("patient_message"):
                self.add(
                    f"{prefix}/issues/{index}/patient_message",
                    "quality_issue",
                    "数据限制",
                    required=True,
                    topic="quality",
