...
class RehabAdapter:
    ENDPOINTS = {
        "rehab.overview": "get_multisource_patient_context",
        "rehab.history": "get_irego_patient_history",
        "rehab.session": "get_irego_session_analysis",
    ...
    async def fetch(self, ctx: RunContext, endpoint: str, args: dict[str, Any]) -> EvidenceEnvelope:
        ...
        return await ctx.repository.save_evidence(
            ctx.scope.scope_hash, endpoint, ctx.record.request_id, body, envelope.contract_version
        )
    ...
    def project(self, task: TaskSpec, evidence: EvidenceEnvelope) -> TaskResult:
        ...
        result = TaskResult(
            task_id=task.task_id, status=status, evidence_ids=[evidence.evidence_id]
        )
        if status in {"failed", "unavailable"}:
            result.code = "tool_" + body["status"]
            result.message = (
                "工具无法提供该结果。" if status == "unavailable" else "工具业务处理失败。"
            )
        data = body.get("data")
        ...
        if endpoint == "get_multisource_patient_context":
            ...
            for key in facts:
                builder.add(
                    f"/data/profile_brief/facts/{pointer_part(key)}", key, key, topic="profile"
                )
            builder.add(
                "/data/data_domains/irego/availability",
                "availability",
                "IREGO数据可用性",
                required=True,
            )
            ...
            for i, item in enumerate(items):
                ...
                for j, _ in enumerate(item.get("display_names") or []):
                    builder.add(
                        f"{prefix}/display_names/{j}", "training_name", f"第{i + 1}条训练项目"
                    )
            if not items:
            ...
            builder.add(
                "/data/overview/total_training_duration",
                "training_duration",
                "训练总时长",
            ...
            if "report_evaluation_facts" in data:
                builder.add(
                    "/data/report_evaluation_facts/completion_rate",
                    "completion_rate",
                    "完成率（比例）",
                )
            ...
            for i, facet in enumerate(data.get("current_performance_facets") or []):
                if facet.get("patient_message"):
                    builder.add(
                        f"/data/current_performance_facets/{i}/patient_message",
                        "current_performance",
                        str(facet.get("capability_name") or "本次表现"),
                        topic="performance",
                    )
            builder.quality()
            result.outputs.update(
                session_ref=ref,
                session_time=times.get(time_field),
                source_version=evidence.source_version,
                record_completed=times.get("training_state") == "completed",
                analysis_evidence=evidence.evidence_id,
                report_available=bool(data.get("report_blocks")),
            )
            ...
            for field, label in [
                ("started_at", "窗口开始"),
                ("ended_at", "窗口结束"),
                ("requested_report_count", "请求记录数"),
                ("resolved_report_count", "窗口记录数"),
            ...
            for i, evaluation in enumerate(data.get("capability_evaluations") or []):
                for field, label in [
                    ("patient_message", "训练表现"),
                    ("explanation_boundary", "解释范围"),
                ]:
                    if evaluation.get(field):
                        builder.add(
                            f"/data/capability_evaluations/{i}/{field}",
                            field,
                            str(evaluation.get("capability_name") or label),
                            required=field == "explanation_boundary",
                            uses=["display", "trend"],
                        )
                ...
                for j, lane in enumerate(project.get("lanes") or []):
                    for k, series in enumerate(lane.get("metric_series") or []):
                        ...
                        for n, _ in enumerate(series.get("comparison_limitations") or []):
                            builder.add(
                                f"{prefix}/comparison_limitations/{n}",
                                "comparison_boundary",
                                "比较限制",
                                required=True,
                            )
                        if eligible and (series.get("interpretation") or {}).get("patient_message"):
                            builder.add(
                                f"{prefix}/interpretation/patient_message",
                                "trend_interpretation",
                                str(series.get("display_name") or "趋势"),
                                uses=["display", "trend"],
                            )
                        for p, point in enumerate(series.get("points") or []):
                            builder.record_ref = point.get("session_ref")
                            field = (
                                "normalized_value"
                                if point.get("normalized_value") is not None
                                else "raw_value"
                            )
                            builder.add(
                                f"{prefix}/points/{p}/{field}",
                                str(series.get("metric_code") or "metric"),
                                (
                                    f"第{point.get('report_ordinal', p + 1)}次"
                                    f"{series.get('display_name') or '指标'}"
                                ),
                                unit=series.get("unit")
                                if series.get("unit_status") == "confirmed"
                                else "unknown",
                                status="valid" if point.get("status") == "valid" else "unknown",
                                uses=["display", "trend"] if eligible else ["display"],
                            )
                        builder.record_ref = None
    ...
    async def execute(self, task: TaskSpec, args: dict[str, Any], ctx: RunContext) -> TaskResult:
        ...
        if task.capability == "rehab.overview":
            ...
            for parent in ctx.record.results.values():
                if parent.outputs.get("session_ref") == args.get(
                    "session_ref"
                ) and parent.outputs.get("analysis_evidence"):
                    evidence = await ctx.repository.evidence(
                        ctx.scope.scope_hash, parent.outputs["analysis_evidence"]
                    )
                    if evidence:
        ...
        cache_key = fingerprint(
            [
                endpoint,
                {**args, "force_refresh": False} if task.capability == "rehab.overview" else args,
            ]
        )
        ...
        result = self.project(task, evidence)
        if (
            task.capability == "rehab.session"
            and result.status in {"succeeded", "partial"}
            and args.get("session_ref")
            and result.outputs.get("session_ref") != args["session_ref"]
        ):
        ...
        if task.effect == "prepare_artifact" and result.status in {"succeeded", "partial"}:
            ...
            if (
                expiry is not None
                and expiry <= utcnow()
                or not await ctx.backend.artifact_available(url)
            ):
                raise DomainError(
    ...
    async def resolve(self, task: TaskSpec, selector: Selector, ctx: RunContext) -> TaskResult:
        ...
        if selector.mode in {"current_ref", "latest_usable"}:
            ...
            args = {
                "selector": "session_ref"
                if anchor and selector.mode == "current_ref"
                else "latest_usable",
        ...
        for page in range(1, 4):
            ...
            target_index = (
                0 if selector.mode in {"none", "latest_record"} else (selector.count or 1) - 1
            )
            if selector.mode == "previous_record":
                position = next(
                    (
                        i
                        for i, (item, _, _) in enumerate(candidates)
        ...
        return TaskResult(
            task_id=task.task_id,
            status="unavailable" if complete else "clarification",
            code="record_not_found",
            message="未找到符合条件的记录。"
            if complete
            else "当前搜索范围内未定位到该记录，请明确日期或记录。",
        )
