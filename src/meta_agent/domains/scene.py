...
class SceneAdapter:
    async def execute(self, task: TaskSpec, args: dict[str, Any], ctx: RunContext) -> TaskResult:
        ...
        body = await ctx.call(
            "navigate_scene", {"target": args["target"], "spaceId": ctx.scope.space_id}
        )
        data = numeric_envelope(body)
        commands = list_field(data, "commands")
        if (
            data.get("spaceId") != ctx.scope.space_id
        ...
        if any(
            type(c.get("order")) is not int or c["order"] != i + 1 for i, c in enumerate(commands)
        ):
            raise DomainError("command_order", "指令结果顺序无效。")
        for command, gid in zip(commands, task.goal_ids, strict=True):
            if source_text(str(command.get("sourceText") or "")) != source_text(
                ctx.goals[gid].query_span
            ):
                raise DomainError(
                    "command_alignment", "工具结果包含未对应到原话的动作。", outcome="clarification"
                )
            if command.get("status") not in {"matched", "ambiguous", "unmatched"}:
                raise DomainError("invalid_command_status", "指令匹配状态无效。")
            if command["status"] == "matched" and not COMMAND.fullmatch(
        ...
        evidence = await ctx.repository.save_evidence(
            ctx.scope.scope_hash, "navigate_scene", ctx.record.request_id, body, "commands-array-v1"
        )
        ...
        for command, gid in zip(commands, task.goal_ids, strict=True):
            ...
            actions[ref] = {
                **command,
                "goal_id": gid,
                "source_space_id": data["spaceId"],
                "scene_version": data["version"],
        ...
        return TaskResult(
            ...
            status="succeeded"
            if all(s == "succeeded" for s in goal_statuses.values())
            else "partial",
            evidence_ids=[evidence.evidence_id],
            outputs={
                "action_ref": references,
                "actions": actions,
                "goal_statuses": goal_statuses,
                "scene_context_confirmed": ctx.scope.scene_version == data["version"],
            },
        )

    async def dispatch(self, task: TaskSpec, action_ref: str, ctx: RunContext) -> TaskResult:
        if ctx.runtime_guard:
            await ctx.runtime_guard()
        if not ctx.settings.scene_actions_enabled:
            raise DomainError("actions_disabled", "动作输出尚未启用。", outcome="unsupported")
        action = next(
            (
                result.outputs.get("actions", {}).get(action_ref)
                for result in ctx.record.results.values()
                if action_ref in result.outputs.get("actions", {})
            ),
            None,
        )
        ...
        if (
            action["source_space_id"] != ctx.scope.space_id
            or action["scene_version"] != ctx.scope.scene_version
        ):
            raise DomainError(
        ...
        if evidence is None:
            raise DomainError(
                "action_expired", "动作来源已过期，请重新匹配。", outcome="clarification"
            )
        exists, raw = resolve_pointer(
            evidence.payload, f"/data/commands/{action['order'] - 1}/result"
        )
        ...
        if raw.startswith(("[Telepor:]", "[ScenePoint:]", "[GamePoint:]")):
            ctx.scene_invalidated = True
        return TaskResult(
            task_id=task.task_id,
            evidence_ids=[evidence.evidence_id],
            message="动作已交给输出通道，执行结果等待客户端确认。",
